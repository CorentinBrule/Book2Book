...
try:
    with open(args.configfile, "r", encoding="utf8") as configfile:
        ...
        svgFontFile = configdata['svgFontFile']
...
for fontStyle in fontStyles :
    ...
    svgGlyphs = [configdata['vectorsFolder'] + "/"+ fontStyle +"/" + f for f in os.listdir(configdata['vectorsFolder'] +"/"+fontStyle) if
                 f.split(".")[-1] == "svg"]
    outputFile = svgFontFile.split(".")[0] + "-" + fontStyle + ".svg"
    ...
    for glyph in svgGlyphs:
        glyphname = glyph.split("/")[-1].split("-")[0]
