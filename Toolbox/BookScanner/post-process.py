...
for i, file in enumerate(sorted(os.listdir(args.path))):
    ...
    if (i < length / 2):
        ...
        name = args.path + file
        subprocess.call(
            ["convert", args.path + file, "-rotate", "-90", "-resize", "{}x{}".format(maxSize[0], maxSize[1]), name])
    else:
        n = int(i - length / 2) * 2
        # name = args.path + str(n)+".jpg"
        name = args.path + file
...
out_files = [f for f in sorted(os.listdir(output_path)) if os.path.isfile(output_path + f)]

for page in output_path:
    if page.split(".")[-1] == "tif":
